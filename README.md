...
> **跨平台加密货币套利系统** — 自动捕捉 CEX 与 DEX 之间的价差，支持现货搬砖、合约-现货对冲、链上 DEX 套利。
...
本系统是一套**全自动加密货币跨平台套利引擎**，核心功能是在多个交易所之间实时发现价差并自动执行买卖，从中赚取差价利润。支持 CEX（Binance、OKX、Bitfinex、Huobi 等）与 DEX（Uniswap），覆盖**现货搬砖**、**合约-现货对冲套利**、**DEX 链上套利**三大策略。
...
│   ├── VirtualTrade.java    # 虚拟平台（用于内部对冲模拟）
